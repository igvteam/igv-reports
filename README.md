...
are described below. Although __--tracks__ is optional, a typical report will include at least an alignment track
...
* One of either **--fasta**, **--twobit**, or **--genome** is required
    ...
    * __--genome__  An igv.js genome identifier (e.g. hg38). If supplied, the sequence, ideogram, and default annotation
      track for the specified genome will be used.

* The arguments -_--begin-_, _---end_-, and _---sequence-_ are required for a generic tab delimited __sites__ file.
...
* Optional
    * __--ideogram__ FILE. Ideogram file in UCSC cytoIdeo format. Useful when __fasta__ is used to specify the
      reference.
    * __--tracks__ LIST. Space-delimited list of track files. See [below](./#track-file-format) for supported formats. If both **--tracks** and
      **--track-config** are specified **--tracks** will appear first by default.
    * __--track-config__  FILE. File containing array of JSON configuration objects to customize the paramaters of igv.js tracks. See [below](./#track-configuration) for more details. When using this option, the track ```url``` and ```indexURL``` properties
    ...
    * __--info-columns-prefixes__ LIST. For VCF based reports only. Space delimited list of prefixes of VCF info field
      ...
      included.
    * __--samples__ LIST. Space delimited list of sample (i.e. genotypes) names. Used in conjunction with 
      **--sample-columns**.
    ...
    * __--subsample__ FLOAT. Output only a portion of input alignments (0.0 -> 1.0). See [samtools documentation](http://www.htslib.org/doc/samtools-view.html)
      for more details
    ...
    * __--merge-overlaps__ Merge overlapping intervals in multi-locus files such as BEDPE. If set, any BEDPE features with
...
Currently supported track file formats are BAM, CRAM, VCF, BED, GFF3, GTF, WIG, and BEDGRAPH. Files in BAM, CRAM, and
VCF format must be indexed. Tabix is supported and it is recommended that all large files be indexed.
...
[igv.js documentation](https://igv.org/doc/igvjs/#tracks/Tracks/).   At a minimum they contain  ```name``` and ```url```
...
specifies only the required properties, while the alignment track includes several additional properties to customize the
